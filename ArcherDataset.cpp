...
void ArcherDataset::calculateINSParameters()
{
	...
	if(rows < 1)
		return;
	if(rows < 100) {
		for(row = 0; rows - 1; row++) {
			this->ins_data[row].heading = M_PI_2 - atan2( this->ins_data[row+1].y - this->ins_data[row].y, 
	...
	/* Most of the rows */
	for(; row < rows - 50; row++) {
		this->ins_data[row].heading = M_PI_2 - atan2( this->ins_data[row+50].y - this->ins_data[row-50].y, 
		                                              this->ins_data[row+50].x - this->ins_data[row-50].x);
	}
